...
Node::Construct (void)
{
  ...
  {
	  decreaseEvent = Simulator::Schedule(Seconds(1.9), &Node::DecreaseEvent_f, this);
  }
  /*if (this->GetId() > 17)
  {
	  decreaseEvent = Simulator::Schedule(Seconds(1.9), &Node::DecreaseEvent_f, this);
	  ratecomputeEvent = Simulator::Schedule(Seconds(1.9), &Node::rate_computing, this);
  }*/
}
...
// tangjian
void Node::DecreaseEvent_f()
{
	lqq_target -=(uint64_t) (lqqtargetdecrease * (double)sequelize_time_per_packet);
	// if the target is below the minnist queue lenth
	if (lqq_target < sequelize_time_per_packet * 20)
		lqq_target = sequelize_time_per_packet * 20;
	resetEvent_f();
}

void Node::IncreaseEvent_f()
{
	lqq_target += (uint64_t)(lqqtargetaddsetp_f * sequelize_time_per_packet);
	lqq_target = lqq_max_target > lqq_target ? lqq_target : lqq_max_target;
	resetEvent_f();
}

void Node::resetEvent_f()
{
	if (decreaseEvent.IsRunning())
		decreaseEvent.Cancel();
	if (Simulator::Now().GetSeconds() > 10.0)
		return;
	decreaseEvent = Simulator::Schedule(NanoSeconds(decreaseinterval*sequelize_time_per_packet), &Node::DecreaseEvent_f, this);
