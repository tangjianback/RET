...
QbbNetDevice::DequeueAndTransmit (void)
{
	...
	if (p != 0) 
	{
		...
		uint16_t protocol_tj = 0;
		ProcessHeader(p, protocol_tj);
		Ipv4Header ipv4h_tj;
		p->RemoveHeader(ipv4h_tj);
		
		if (p->GetSize() > 1000)
		{
			if (p->GetSize() != 1031 && p->GetSize() != 1053)
				std::cout << "error in set depatring time" << std::endl;
		}
		//nic 是 0 ,switch 是 1
		if ((ipv4h_tj.GetProtocol() != 0xFF && ipv4h_tj.GetProtocol() != 0xFD && ipv4h_tj.GetProtocol() != 0xFC) && p->GetSize() >= 1000)
		{
			 // this code only when the nic is on endpoint and is delivering to the dest nic works
			if (ipv4h_tj.GetProtocol() != 0xFE && m_node->GetNodeType() == 0 && ipv4h_tj.GetProtocol() == 17) //not PFC and is nic and look for udp
			{
				UdpHeader udph_tj;
				p->RemoveHeader(udph_tj);

				SeqTsHeader sth_tj;
				p->RemoveHeader(sth_tj);

				if (sth_tj.GetAckNeeded() == 1)
				{
					//std::cout << "set send time" << std::endl;
					sth_tj.SetTsAsUint64(Simulator::Now().GetInteger());
				}
				p->AddHeader(sth_tj);
				p->AddHeader(udph_tj);
			}
		}
		p->AddHeader(ipv4h_tj);
		AddHeader(p, protocol_tj);
		//endtangjian
		...
		else //I am a switch, do ECN if this is not a pause
		{
			...
			else
			{
				...
				if (true)
				{
					...
					//nic 是 0 ,switch 是 1
					if ((ipv4h_tj.GetProtocol() != 0xFF && ipv4h_tj.GetProtocol() != 0xFD && ipv4h_tj.GetProtocol() != 0xFC))
					{
						
						// this code only when the nic is on endpoint and is delivering to the dest nic works
						if (ipv4h_tj.GetProtocol() != 0xFE && ipv4h_tj.GetProtocol() == 17) //not PFC and is nic and look for udp
						{
							...
							if (sth_tj.GetAckNeeded() == 1)
							{
								...
								// indev 来的端口 m_ifIndex去的端口 lastqueue 是优先级
								uint64_t ecn_if = (uint64_t)ShouldSendCN_my(inDev, m_ifIndex, m_queue->GetLastQueue());
								sth_tj.SetTargetAsUint64(ecn_if);
							}
							p->AddHeader(sth_tj);
							p->AddHeader(udph_tj);
						}
...
bool
QbbNetDevice::ShouldSendCN_my(uint32_t indev, uint32_t ifindex, uint32_t qIndex)
{
	return m_node->m_broadcom->ShouldSendCN_my(indev, ifindex, qIndex);
	//std::cout << indev << " " << ifindex << " " << qIndex << std::endl;
	/*if (this->GetNode()->GetId() >= 6 && this->GetNode()->GetId() <= 17)
		return m_node->m_broadcom->ShouldSendCN_my(indev, ifindex, qIndex);
	else
		return false;*/
}
